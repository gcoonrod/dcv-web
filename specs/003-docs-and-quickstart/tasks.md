...
- [x] T009 Create `src/pages/docs/[...slug].astro` — `getStaticPaths` calls `getCollection('docs')` and maps each entry to `{ params: { slug: entry.id }, props: { entry } }`; page frontmatter destructures `entry` from `Astro.props` and `slug` from `Astro.params`; calls \`render(entry)\` (Astro 5 Content Layer helper) to get \`Content\`; renders `<DocsLayout title={entry.data.title} description={entry.data.description} currentSlug={slug}><Content /></DocsLayout>` (contracts/component-api.md §Contract 4)
